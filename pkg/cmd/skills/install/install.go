...
func NewCmdInstall(f *cmdutil.Factory, telemetry ghtelemetry.CommandRecorder, runF func(*InstallOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			When run non-interactively without a skill name, the matching skills are
			listed (as tab-separated values when piped) so you can browse or filter
			them with tools like %[1]sgrep%[1]s before re-running with a specific skill.
